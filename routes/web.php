...
Route::middleware(['auth', 'verified'])->group(function () {
    ...
    Route::get('/theses/create', [ThesisController::class, 'create'])
    ...
    Route::patch('/theses/{thesis}/status', [ThesisController::class, 'updateStatus'])
        ->middleware('role:professor,admin')
        ->name('theses.status.update');

    Route::post('/theses/{thesis}/versions', [ThesisVersionController::class, 'store'])
        ->middleware('role:student')
        ->name('thesis-versions.store');

    Route::get('/theses/{thesis}/versions/{version}/download', [ThesisVersionController::class, 'download'])
        ->name('thesis-versions.download');

    Route::post('/theses/{thesis}/feedback', [FeedbackController::class, 'store'])
    ...
    Route::get('/theses/{thesis}/chat', [ChatController::class, 'show'])
        ->name('chat.show');

    Route::post('/theses/{thesis}/chat', [ChatController::class, 'store'])
        ->name('chat.store');
    ...
    Route::prefix('admin')
        ...
        ->group(function () {
            ...
            Route::patch('/approvals/{user}/approve', [UserApprovalController::class, 'approve'])->name('approvals.approve');
            Route::patch('/approvals/{user}/reject', [UserApprovalController::class, 'reject'])->name('approvals.reject');
